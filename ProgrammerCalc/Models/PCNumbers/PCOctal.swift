...
//  ProgrammerCalc
//
//  Created by Ivan Ghiba on 01.12.2023.
//  Copyright © 2023 ighiba. All rights reserved.
//

import Foundation

struct PCOctal: PCNumber, PCNumberDigits, PCNumberBinaryRepresentatable {
    ...
    private let octHelper: OctHexHelper = OctHexHelper(bitTable: .oct)
    private let bitsInDigitCount: Int = 3
    ...
    init(pcDecimal: PCDecimal, bitWidth: UInt8, isSigned: Bool) {
        let pcDecimalFixed = pcDecimal.fixedOverflow(bitWidth: bitWidth, isSigned: isSigned)
        self.setupFrom(pcDecimal: pcDecimalFixed, bitWidth: bitWidth)
    }
    ...
    init?(_ description: String) {
        if let parsedResult = octHelper.parseInputString(description) {
    ...
    func formatted() -> String {
        ...
        return composeOctalString(integerString: integerString, fractionalString: fractionalString)
    }
    ...
    func formattedOutput(bitWidth: UInt8, fractionalWidth: UInt8) -> String {
        let contractedIntegerBits = intPart.bits.contracted(toCount: Int(bitWidth))
    ...
    private func composeOctalString(integerString: String, fractionalString: String) -> String {
    ...
    private func convertToIntegerDigits(from bits: BitArray) -> DigitArray {
        var integerDigits = octHelper.convertToIntegerDigits(from: bits).removedTrailingZeroDigits()
    ...
    private func convertToFractionalDigits(from bits: BitArray) -> DigitArray {
        return octHelper.convertToFractionalDigits(from: bits).removedTrailingZeroDigits()
    }
    ...
    mutating func appendIntegerDigit(_ digit: Digit, bitWidth: UInt8, isSigned: Bool) {
        ...
        let bits: [Bit] = octHelper.convertDigitToBits(digit).reversed()
